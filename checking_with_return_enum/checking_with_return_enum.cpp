...
Response execute(const Request& req) {
    ...
    if(response_error rc = check_bad_response(resp); rc != response_error::success) {
      std::cerr << "Bad response : " << static_cast<typename std::underlying_type<response_error>::type>(rc) 
         << " -> " << resp << std::endl;
